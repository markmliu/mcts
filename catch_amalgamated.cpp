...
public:
  explicit TestGroup(IStreamingReporterPtr &&reporter, Config const *config)
      : m_reporter(reporter.get()), m_config{config}, m_context{
                                                          config,
                                                          std::move(reporter)} {
...
makeTestCaseInfo(std::string const &_className, NameAndTags const &nameAndTags,
...
TestCaseInfo::TestCaseInfo(std::string const &_className,
    ...
    : name(_nameAndTags.name.empty() ? makeDefaultName() : _nameAndTags.name),
      className(_className), lineInfo(_lineInfo) {
...
TestSpec::Matches
TestSpec::matchesByFilter(std::vector<TestCaseHandle> const &testCases,
...
Arg::parse(std::string const &, Detail::TokenStream const &tokens) const {
...
Detail::InternalParseResult
Opt::parse(std::string const &, Detail::TokenStream const &tokens) const {
  auto validationResult = validate();
  if (!validationResult)
    return Detail::InternalParseResult(validationResult);
...
Detail::InternalParseResult
ExeName::parse(std::string const &, Detail::TokenStream const &tokens) const {
  return Detail::InternalParseResult::ok(
...
std::string
Catch::Matchers::Detail::finalizeDescription(const std::string &desc) {
  if (desc.empty()) {
